...
public class BasePage : System.Web.UI.Page
{
    ...
    private bool GetRole(string dwbm, string gh)
    {
        ...
        if (path == "pages/gngl/gnmanager.aspx" || path == "pages/qxgl/rolemanager.aspx" || path == "pages/gngl/anmanager.aspx")
        {
        ...
        if (path == "pages/business/fullqueryreadingfile.aspx" || path == "pages/business/fullquerydetailed.aspx")
        {
            path = "pages/business/fullquery.aspx";
        }
    ...
    /// </summary>
    /// <returns></returns>
    public string GetJsQxzt()
    {       
        string qxzt = "";
        var qxztList = UserRole.Where(p => string.IsNullOrEmpty(p.QXZT)).ToList();
        if (qxztList.Count == 0)
        {
            qxztList = UserRole.Where(p => p.QXZT == "B").ToList();
            if (qxztList.Count == 0)
            {
                qxztList = UserRole.Where(p => p.QXZT == "G").ToList();
                if (qxztList.Count > 0)
                    qxzt = "G";
            }
            else
