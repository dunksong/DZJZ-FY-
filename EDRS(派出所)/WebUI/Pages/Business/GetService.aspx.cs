            ...
            if (!IsPostBack)
            {
                ...
                if (type == "aj")
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        Response.Write(ReturnString.JsonToString(Prompt.error,ex.Message+",无法远程连接计算机", null));
                        Response.End();
                    }
                ...
                else if (type == "ws")
                {
                    ...
                    DataTable dt2 = JsonHelper.ToDataTable(row2);
                    DataView view2 = dt2.DefaultView;
                    view2.RowFilter = String.Format("wSMC = '{0}' or wSMC2 = '{1}' or wSMC = '{2}' or wSMC = '{3}' or wSMC = '{4}' or wSMC = '{5}' or wSMC = '{6}' or wSMC = '{7}'", "起诉意见书", "补充侦查报告书", "要求复议意见书", "提请复核意见书", "呈请延长侦查羁押期限报告书", "呈请重新计算侦查羁押期限报告书（逮捕）", "提请批准逮捕书", "呈请变更逮捕措施报告书");
                    DataTable dtNew2 = view2.ToTable();
                    s = "{\"Rows\":" + JsonHelper.JsonString(dtNew2) + "}";
                    ...
                    if (!string.IsNullOrEmpty(condition2) && condition2 != "[]")
                    {
                        string v = JsonHelper.DeserializeObjectKey(condition2, "value");
                        view2.RowFilter = String.Format("wRITID like '%{0}%' or wSMC like '%{0}%' or zXDXJY like '%{0}%'", v);
                        dtNew2 = view2.ToTable();
                        s = "{\"Rows\":" + JsonHelper.JsonString(dtNew2) + "}";
                   ...
                   */
                    ...
                    {
                        LogHelper.LogError(this.Request, "访问文书接口错误：", ex.Message, "", "");
                    ...
                    using (WebResponse wr = req.GetResponse())
                    {
                        ...
                        try
                        {
                            ...
                            yhtml = strhtml;
                            //LogHelper.LogError(this.Request, "TestWS原数据" + ajbh, strhtml, "", "");
                            string row = JsonHelper.DeserializeObjectKey("[" + strhtml + "]", "Rows");
                            dt = JsonHelper.ToDataTable(row);
                            if (dt != null && dt.Rows.Count > 0)
                            {
                                ...
                                Response.Write(ReturnString.JsonToString(Prompt.error,"该案件没有文书", null));
                        }
                        catch (Exception ex)
                        {
                            string temp = "";
                            ...
                            LogHelper.LogError(this.Request, "TableName", temp + " 案件编号：" + ajbh,"原来数据："+ yhtml, "");
                            Response.Write(ReturnString.JsonToString(Prompt.error, ex.Message, null));
                        }
