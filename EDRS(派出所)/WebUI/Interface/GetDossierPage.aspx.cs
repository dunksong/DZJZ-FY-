...
namespace WebUI.Interface
{
    ...
    public partial class GetDossierPage : System.Web.UI.Page
    {
        ...
        public string GetDossierDoInfo()
        {
          ...
          //  DataSet ds = bll.GetJzjbxxByBmsah(bh, dwbm);
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                   ...
                   // dtData.Columns.Add("nocheck");
                    ...
                    for (int i = 0; i < dtData.Rows.Count; i++)
                    {
                        ...
                        dtData.Rows[i]["zzzt"] = list[0].ZZZT;// (list[0].ZZZT == "2" ? true : false);
            ...
            {
                LogHelper.LogError(Request, "Exception", ex.Message, "public string GetDossierFileInfo()", "GetDossierPage");
                return ReturnString.JsonToString(Prompt.error, "查询失败", null);
            }
        }
        #endregion
	...
	{	        
        ...
        public string GetDossierFilePage()
        {
            ...
            {
                LogHelper.LogError(Request, "Exception", ex.Message, "public string GetDossierFilePage()", "GetDossierPage");
                return ReturnString.JsonToString(Prompt.error, "查询失败", null);
            }
        }
        #endregion
        ...
        public string GetDossierFilePageInfo()
        {
            try
            {
                ...
                list = list.Where(p => p.JZBH.Trim() == jzbh).ToList<EDRS.Model.YX_DZJZ_JZJBXX>();
                bh = list[0].BMSAH;
                //返回参数
                string Str = string.Empty;
                EDRS.BLL.YX_DZJZ_JZML bll = new EDRS.BLL.YX_DZJZ_JZML(HttpContext.Current.Request);
            ...
            {
                LogHelper.LogError(Request, "Exception", ex.Message, "public string GetDossierFilePageInfo()", "GetDossierPage");
                return ReturnString.JsonToString(Prompt.error, "文件查询失败", null);
            }
        }
