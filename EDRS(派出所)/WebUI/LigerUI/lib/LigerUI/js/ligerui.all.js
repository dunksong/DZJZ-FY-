    ...
    $.ligerui.controls.Accordion.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            $(">.l-accordion-header", g.accordion).click(function ()
            {
                ...
                {
                //点击菜单增加和移除l-accordion-header-inner的css
                    $(".l-accordion-header-inner").removeClass("l-accordion-header-inner-close");
                    $(".l-accordion-header-inner").addClass("l-accordion-header-inner-open");
                    $(this).next(".l-accordion-content")
                    .show(p.speed)
                    .siblings(".l-accordion-header-inner-content:visible").hide(p.speed);
                    $(this).siblings(".l-accordion-header").find(".l-accordion-header-inner").removeClass("l-accordion-header-inner-open").addClass("l-accordion-header-inner-close");
                    //
                ...
                //
                    $(".l-accordion-header-inner").addClass("l-accordion-header-inner-close");
                    $(".l-accordion-header-inner").removeClass("l-accordion-header-inner-open");
...
(function ($)
{
    ...
    {
        content = content || "";        
        var s = content.split("fn=");
        if (s.length > 0 && callback == null) {
            content = s[0];
            callback = eval(s[1]);
        }
...
(function ($)
{
    ...
    $.ligerDefaults.GridString = {
        errorMessage: '发生错误',
       // pageStatMessage: '显示从{from}到{to}条，共 {total} 条 。每页显示：{pagesize}',
       pageStatMessage:'每页显示：<label class="show_tiao">{pagesize}条</label>,共 {total} 条',
    ...
    //$(".l-bar-selectpagesize select").hide();
        ...
        */
        ...
        _initFootbar: function ()
        {
            ...
            if (p.usePager)
            {
                ...
                $('.l-bar-selectpagesize', g.toolbar).append("<select class='remove_select' name='rp'>" + optStr + "</select>");
        ...
        _getCellHtml: function (rowdata, column)
        {
            ...
            htmlarr.push(' style = "white-space:nowrap;width:' + parseInt(column._width - 8) + 'px;');
        ...
        _getTotalCellContent: function (column, data)
        {
            ...
            if (column.totalSummary)
            {
                ...
                else if (column.totalSummary.type)
                {
                    ...
                    if (isExist('sum'))
                        totalsummaryArr.push("<div>总：" + sum + "</div>");
                    if (isExist('count'))
...
(function ($)
{
    ...
    $.ligerui.controls.RadioList.ligerExtend($.ligerui.controls.Input, {
        ...
        {
            var g = this, p = this.options;
            var value =p.value|| g.valueField.val() || g._getValue();
            g._changeValue(value);
        },
        //设置值到 隐藏域
        _changeValue: function (newValue)
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        append: function (parentNode, newdata, nearNode, isAfter) {
            ...
            if (parentNode == null)//增加到根节点
            {
                ...
                if (p.nodeHeight)
                    $(g.tree).parent()[0].attributes[1].nodeValue = "height:auto; max-height: " + p.nodeHeight + "px;";
                if (nearNode != null) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (g.hasChildren(o)) {
                    ...
                    if (p.parentIcon) {
                        ...
                        treehtmlarr.push('">');
                        if (p.iconFieldName && o[p.iconFieldName])
                            treehtmlarr.push('<img src="' + eval(o[p.iconFieldName]) + '" />');
                ...
                if (p.render) {
                    ...
                    treehtmlarr.push('<span style="'+o.style+'" >' + o[p.textFieldName] + '</span>');
                }
                treehtmlarr.push('</div>');
                if (g.hasChildren(o)) {
                    var isLastNew = [];
