...
(function ($) {
    ...
    $.ligerDefaults.Grid = {
        ...
        pageSizeOptions: [20, 50, 100, 500],  //可选择设定的每页结果数
        parms: [],                         //提交到服务器的参数
        columns: [],                          //数据源
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        */
        ...
        toggle: function (rowParm, treelink) {
            if (!rowParm) return;
            var g = this, p = this.options;
            var rowdata = g.getRow(rowParm);
            ...
            for (var i = 0, l = children.length; i < l; i++) {
                ...
                if (opening) {
                    if (treelink) {
                        var cp = g.getParent(o);
                        var cc = g.getChildren(o, false);
                        if (cp == rowdata) {
                            if (cc.length == 0)
        ...
        _getHtmlFromData: function (data, frozen) {
            ...
            for (var i = 0, l = data.length; i < l; i++) {
                ...
                $(g.columns).each(function (columnindex, column) {
                    ...
                    gridhtmlarr.push('width:' + colwidth + 'px; '); //内容列宽修改列宽
                    //gridhtmlarr.push('width:auto; ');
        ...
        _getCellHtml: function (rowdata, column) {
            ...
            //htmlarr.push('<div');
            //htmlarr.push(' style = "width:' + parseInt(column._width - 8) + 'px;'); //内容列div宽度修改列宽
            htmlarr.push(' style = "white-space:nowrap;width:' + parseInt(column._width - 8) + 'px;');
            //htmlarr.push(' style = "width:auto; white-space:nowrap; min-width:' + parseInt(column._width - 8) + 'px;');
        ...
        _getTotalCellContent: function (column, data) {
            ...
            if (column.totalSummary) {
                ...
                if (column.totalSummary.render) {
                    ...
                    if (isExist('sum'))
                        totalsummaryArr.push("<div>" + sum + "</div>");
                    if (isExist('count'))
                        totalsummaryArr.push("<div>总数：" + count + "</div>");
                    if (isExist('max'))
                        totalsummaryArr.push("<div>最大：" + max + "</div>");
                    if (isExist('min'))
                        totalsummaryArr.push("<div>最小" + min + "</div>");
                    if (isExist('avg'))
                        totalsummaryArr.push("<div>平均：" + avg.toFixed(2) + "</div>");
                }
            }
