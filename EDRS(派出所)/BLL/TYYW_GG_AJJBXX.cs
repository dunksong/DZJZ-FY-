...
namespace EDRS.BLL
{
	...
	public partial class TYYW_GG_AJJBXX
	{
        ...
        /// <param name="gh"></param>
        /// <returns></returns>
        public DataTable ListBinTable(System.Web.HttpRequest Request, string dwbm, string gh,string qxzt,string bmbm)
        {
            int count = 0;
            string msg = "";
            //读取配置文件中的配置
            ...
            {
                return ListBind(Request, dwbm, gh, qxzt,bmbm,"", ref count);
        ...
        /// <param name="gh"></param>
        /// <returns></returns>
        public DataTable ListBind(System.Web.HttpRequest Request, string dwbm, string gh,string qxzt, string bmbm,string no,ref int count)
        {
            string page = Request["page"];
            string rows = Request["pagesize"];
           ...
           // StringBuilder strWhere = new StringBuilder();
            ...
            //根据角色权限状态判断获取数据
            if (!string.IsNullOrEmpty(qxzt))
            {
                if (qxzt == "G")
                    where += " and JZSCRGH='" + gh + "'";
                else if (qxzt == "B")
                {
                    XT_QX_RYJSFP ryjsBll = new XT_QX_RYJSFP(Request);
                    DataSet dsryjs = ryjsBll.GetListChildrenBmAll(" and BMBM =:BMBM and SFSC='N'", new object[] { bmbm });
                    if (dsryjs != null && dsryjs.Tables.Count > 0 && dsryjs.Tables[0].Rows.Count > 0)
                    {
                        where += " and JZSCRGH in (";
                        int num=0;
                        foreach (DataRow dr in dsryjs.Tables[0].Rows)
                        {
                            where += "'" + dr["GH"] + "'";
                            if (num < dsryjs.Tables[0].Rows.Count - 1)
                                where += ",";
                            num++;
                        }
                        where += ") ";
