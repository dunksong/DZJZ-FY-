...
namespace EDRS.Common
{
    ...
    public class WebClient
    {
        ...
        public string Post(string url, Dictionary<String, String> dicList,Encoding encoding)
        {
            System.Net.HttpWebRequest request = (System.Net.HttpWebRequest)System.Net.WebRequest.Create(url);
            request.AllowAutoRedirect = false;
            request.AllowWriteStreamBuffering = false;
            ...
            try
            {
                ...
                System.Net.HttpWebResponse response = (System.Net.HttpWebResponse)request.GetResponse();
                System.IO.StreamReader myStreamReader = new System.IO.StreamReader(response.GetResponseStream(),encoding );
                var retString = myStreamReader.ReadToEnd();
                myStreamReader.Close();
        ...
        public static String buildQueryStr(Dictionary<String, String> dicList)
        {
            ...
            {
                postStr += item.Key + "=" + item.Value + "&"; //System.Web.HttpUtility.UrlEncode(item.Value, Encoding.UTF8)
            }
