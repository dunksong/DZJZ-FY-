...
namespace Maticsoft.Common
{
    ...
    public class DataToExcel
    {
        ...
        */
        ...
        /// <returns></returns>
        public string DataExcel(System.Data.DataTable dt, string strTitle, string FilePath, Dictionary<string, string> nameList, Dictionary<string, object> valueList)
        {
            ...
            //{
            foreach (KeyValuePair<string, string> colhead in nameList)
            {
                if (dt.Columns.Contains(colhead.Key))
                //if (nameList.ContainsKey(colhead.ColumnName))
                {
                    colIndex++;
                    string name = colhead.Key.Trim();
                    object namestr = (object)name;
                    namestr = colhead.Value;
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                //foreach (DataColumn col in dt.Columns)
                //{
                //    if (nameList.ContainsKey(col.ColumnName))
                foreach (KeyValuePair<string, string> colhead in nameList)
                {
                    if (dt.Columns.Contains(colhead.Key))
                    {
                        colIndex++;
                        if (dt.Columns[colhead.Key].DataType == System.Type.GetType("System.DateTime"))
                        {
                            object str = (object)(Convert.ToDateTime(row[colhead.Key].ToString())).ToString("yyyy-MM-dd");
                            excel.WriteValue(ref vt, ref cf, ref ca, ref chl, ref rowIndex, ref colIndex, ref str, ref cellformat);
                        }
                        else
                        {
                            object str;
                            if (valueList != null && valueList.ContainsKey(colhead.Key))
                            {
                                object va;
                                if (valueList.TryGetValue(colhead.Key, out va))
                                {
                                    if (va.GetType().Name == "Dictionary`2")
                                        ((Dictionary<string, object>)va).TryGetValue(row[colhead.Key].ToString().ToLower(), out str);
                                    else
                                        str = (object)row[colhead.Key].ToString() + va.ToString();
                                }
                                else
                                    str = (object)row[colhead.Key].ToString();
                            }
                            else
                                str = (object)row[colhead.Key].ToString();
                            excel.WriteValue(ref vt, ref cf, ref ca, ref chl, ref rowIndex, ref colIndex, ref str, ref cellformat);
                ...
                //        {
                //            object str;
                //            if (valueList != null && valueList.ContainsKey(col.ColumnName))
                //            {
                //                object va;
                //                if (valueList.TryGetValue(col.ColumnName, out va))
                //                {
                //                    if (va.GetType().Name == "Dictionary`2")
                //                        ((Dictionary<string, object>)va).TryGetValue(row[col.ColumnName].ToString().ToLower(), out str);
                //                    else
                //                        str = (object)row[col.ColumnName].ToString() + va.ToString();
                //                }
                //                else
