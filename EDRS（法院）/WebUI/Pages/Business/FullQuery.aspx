    ...
    <div id="searchbar">
        <table class="searchbartab" border="0">
            <tr>
                ...
                <td colspan="4" style="">
                    <input id="txt_query" class="l-text" type="text" name="txt_query" style="width: 283px;" />
                </td>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            var grid = $("#mainGrid").ligerGrid({
                width: '100%', height: '100%', heightDiff: -42, usePager: false
            });
        ...
        function search(rows, type) {
            if ($("#txt_query").val() == "" && $("#txt_key").val() == "" && $("#txt_name").val() == "" && $("#txt_dutyman").val() == "" && $("#txt_time_end").val() == "") {
                $(".l-grid-body").html("<span style=\" font-size:14px;\" >请输入检索条件.<span>");
                return;
            }
            $.AjaxPost({
                url: "FullQuery.aspx",
                data: { t: "querydata", txt: $("#txt_query").val(), rows: rows, bmsah: $("#txt_key").val(), ajmc: $("#txt_name").val(), cbr: $("#txt_dutyman").val(), slrq1: $("#txt_time_begin").val(), slrq2: $("#txt_time_end").val() },
                success: function (data) {
                    ...
                    if (data.response) {
                        ...
                        $.each(data, function (i, n) {
                            ...
                            html += "<a href=\"javascript:\" onclick=\"parent.f_addTab('" + n.id + "','" + n.WJMCDZJZ + n.WJHZ + "', '/Pages/Business/FullQueryDetailed.aspx?id=" + n.id + "');\" >" + n.WJMCDZJZ + n.WJHZ /*n.title[0]*/ + "</a></div>";
                            if (datato["" + n.id + ""].text)
