...
namespace WebUI
{
    public partial class FullQuery : BasePage
    {
        ...
        private string QueryData()
        {
            ...
            {
                jsonParas += " AND BMSAH:\"" + bmsah+"\"~";
            }
            //案件名称
            if (!string.IsNullOrEmpty(ajmc))
            {
                jsonParas += " AND AJMC:" + ajmc+"~";
            }
            //承办人
            if (!string.IsNullOrEmpty(cbr))
            {
                jsonParas +=" AND CBR:" + cbr+"~";
            }
            //受理开始日期
            if (!string.IsNullOrEmpty(slrq1))
            {
                if (!string.IsNullOrEmpty(slrq1) && !string.IsNullOrEmpty(slrq2))
                    jsonParas += " AND ZHXGSJ:[\"" + Convert.ToDateTime(slrq1).ToString("yyyy-MM-ddTHH:mm:ss" + "Z") + "\" TO \"" + Convert.ToDateTime(slrq2).AddDays(1).ToString("yyyy-MM-ddTHH:mm:ss" + "Z") + "\"]";
                else if(!string.IsNullOrEmpty(slrq1) && string.IsNullOrEmpty(slrq2))
                    jsonParas += " AND ZHXGSJ:[\"" + Convert.ToDateTime(slrq1).ToString("yyyy-MM-ddTHH:mm:ss" + "Z") + "\" TO \"" + DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss" + "Z") + "\"]";
                //["2016-01-16T00:00:00Z" TO "2016-01-16T23:59:59Z"]             
            }
            if (!string.IsNullOrEmpty(txt))
            {
                jsonParas += " AND WSCFLJ:" + txt + "~";
            ...
            if (!string.IsNullOrEmpty(jsonParas))
            {
                ...
                jsonParas = System.Web.HttpUtility.UrlEncode(jsonParas); //+ " AND parent:VIEW_DZJZ"
