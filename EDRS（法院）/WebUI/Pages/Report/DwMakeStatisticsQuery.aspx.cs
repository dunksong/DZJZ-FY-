...
namespace WebUI.Pages.Report
{
    public partial class DwMakeStatisticsQuery : BasePage
    {
        ...
        public string ListBind()
        {
            ...
            {
                where += " and b.CBR like '%" + StringPlus.ReplaceSingle(dutyman) + "%'";
            }
            ...
            {
                where += " and dk.GDRQ >= to_date('" + StringPlus.ReplaceSingle(timebegin) + "','yyyy-mm-dd')";
            }
            if (!string.IsNullOrEmpty(timeend))
            {
                where += " and dk.GDRQ <= to_date('" + Convert.ToDateTime(StringPlus.ReplaceSingle(timeend)).AddDays(1).ToString("yyyy-MM-dd") + "','yyyy-mm-dd')";
            }
        ...
        private string DeriveData()
        {
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    msg = DataToExcel_Ex.Export(dtCopy, "卷宗制作情况", Server.MapPath("/" + ExcelFolder + "/" + filename));
