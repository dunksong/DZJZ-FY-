...
namespace WebUI.Handler.ZZJG
{
    ...
    public class DZJZ_Report : AshxBase
    {
        ...
        {
            EDRS.BLL.XT_DM_YWBM bll = new EDRS.BLL.XT_DM_YWBM(context.Request);
            DataSet ds = bll.GetList(" and SFSC='N' order by XH asc");
            if (ds != null && ds.Tables.Count > 0)
            {
        ...
        private void GetYearList(HttpContext context)
        {
            ...
            bool isExp = true;
            int n = DateTime.Now.Year+1;
            while (isExp)
            {
                str.Append("{\"id\":\"" + n + "\",\"text\":\"" + n + "\"},");
