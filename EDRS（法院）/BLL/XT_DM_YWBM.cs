...
namespace EDRS.BLL
{
	...
	{
		private readonly IXT_DM_YWBM dal=DataAccess.CreateXT_DM_YWBM();
		public XT_DM_YWBM(System.Web.HttpRequest _context)
        {
		...
		public EDRS.Model.XT_DM_YWBM GetModel(string YWBM)
		{
			...
			return dal.GetModel(YWBM);
		}

		/// <summary>
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public EDRS.Model.XT_DM_YWBM GetModelByCache(string YWBM)
		{

            string CacheKey = "XT_DM_YWBMModel-" + YWBM;
			object objModel = EDRS.Common.DataCache.GetCache(CacheKey);
			if (objModel == null)
			{
				try
				{
					objModel = dal.GetModel(YWBM);
					if (objModel != null)
					{
			...
			return (EDRS.Model.XT_DM_YWBM)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
        public List<EDRS.Model.XT_DM_YWBM> GetModelList(string strWhere, params object[] objValues)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<EDRS.Model.XT_DM_YWBM> DataTableToList(DataTable dt)
		{
			List<EDRS.Model.XT_DM_YWBM> modelList = new List<EDRS.Model.XT_DM_YWBM>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				EDRS.Model.XT_DM_YWBM model;
        ...
        {
            return dal.DeleteList(ywbmList);
        }
