...
namespace EDRS.BLL
{
	...
	{
		private readonly IYX_DZJZ_JZMLWJ dal=DataAccess.CreateYX_DZJZ_JZMLWJ();
        public YX_DZJZ_JZMLWJ(System.Web.HttpRequest _context)
        {
		...
		{
			return dal.Exists(JZBH,MLBH,WJXH);
		}
		...
		public bool Delete(string JZBH,string MLBH,string WJXH)
		{
			...
			return dal.Delete(JZBH,MLBH,WJXH);
		}
		...
		public EDRS.Model.YX_DZJZ_JZMLWJ GetModel(string JZBH,string MLBH,string WJXH)
		{
			...
			return dal.GetModel(JZBH,MLBH,WJXH);
		}

		/// <summary>
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public EDRS.Model.YX_DZJZ_JZMLWJ GetModelByCache(string JZBH,string MLBH,string WJXH)
		{
			
			string CacheKey = "YX_DZJZ_JZMLWJModel-" + JZBH+MLBH+WJXH;
			object objModel = EDRS.Common.DataCache.GetCache(CacheKey);
			if (objModel == null)
			{
				try
				{
					objModel = dal.GetModel(JZBH,MLBH,WJXH);
					if (objModel != null)
					{
			...
			return (EDRS.Model.YX_DZJZ_JZMLWJ)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
        public List<EDRS.Model.YX_DZJZ_JZMLWJ> GetModelList(string strWhere, params object[] objValues)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<EDRS.Model.YX_DZJZ_JZMLWJ> DataTableToList(DataTable dt)
		{
			List<EDRS.Model.YX_DZJZ_JZMLWJ> modelList = new List<EDRS.Model.YX_DZJZ_JZMLWJ>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				EDRS.Model.YX_DZJZ_JZMLWJ model;
