using System;
using System.Data;
using System.Collections.Generic;
using EDRS.Common;
using EDRS.Model;
using EDRS.DALFactory;
using EDRS.IDAL;
namespace EDRS.BLL
{
	/// <summary>
	/// 律师阅卷绑定表(一个
	/// </summary>
	public partial class YX_DZJZ_LSAJBD
	{
		private readonly IYX_DZJZ_LSAJBD dal=DataAccess.CreateYX_DZJZ_LSAJBD();
        public YX_DZJZ_LSAJBD(System.Web.HttpRequest _context)
        {
            dal.SetHttpContext(_context);
        }
		#region  BasicMethod
		/// <summary>
		/// 是否存在该记录
		/// </summary>
		public bool Exists(string YJXH)
		{
			return dal.Exists(YJXH);
		}

		/// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(EDRS.Model.YX_DZJZ_LSAJBD model)
		{
			return dal.Add(model);
		}
        /// <summary>
        /// 增加多条数据
        /// </summary>
        public bool AddList(EDRS.Model.YX_DZJZ_LSYJSQ lsyjsqModel, EDRS.Model.YX_DZJZ_LSAJBD lsajbdModel, List<EDRS.Model.YX_DZJZ_LSAJWJFP> modelList, EDRS.Model.YX_DZJZ_LSGL lsglModel)
        {
            return dal.AddList(lsyjsqModel, lsajbdModel, modelList, lsglModel);
        }
		/// <summary>
		/// 更新一条数据
		/// </summary>
		public bool Update(EDRS.Model.YX_DZJZ_LSAJBD model)
		{
			return dal.Update(model);
		}

		/// <summary>
		/// 删除一条数据
		/// </summary>
		public bool Delete(string YJXH)
		{
			
			return dal.Delete(YJXH);
		}

        /// <summary>
        /// 逻辑删除
        /// </summary>
        /// <param name="ids"></param>
        /// <returns></returns>
        public bool DeleteList(string ids)
        {
            return dal.DeleteList(ids);
        }

		/// <summary>
		/// 得到一个对象实体
		/// </summary>
		public EDRS.Model.YX_DZJZ_LSAJBD GetModel(string YJXH)
		{
			
			return dal.GetModel(YJXH);
		}

		/// <summary>
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public EDRS.Model.YX_DZJZ_LSAJBD GetModelByCache(string YJXH)
		{
			
			string CacheKey = "YX_DZJZ_LSAJBDModel-" + YJXH;
			object objModel = EDRS.Common.DataCache.GetCache(CacheKey);
			if (objModel == null)
			{
				try
				{
					objModel = dal.GetModel(YJXH);
					if (objModel != null)
					{
						int ModelCache = EDRS.Common.ConfigHelper.GetConfigInt("ModelCache");
						EDRS.Common.DataCache.SetCache(CacheKey, objModel, DateTime.Now.AddMinutes(ModelCache), TimeSpan.Zero);
					}
				}
				catch{}
			}
			return (EDRS.Model.YX_DZJZ_LSAJBD)objModel;
		}

		/// <summary>
		/// 获得数据列表
		/// </summary>
        public DataSet GetList(string strWhere, params object[] objValues)
		{
            return dal.GetList(strWhere, objValues);
		}
		/// <summary>
		/// 获得数据列表
		/// </summary>
        public List<EDRS.Model.YX_DZJZ_LSAJBD> GetModelList(string strWhere, params object[] objValues)
		{
            DataSet ds = dal.GetList(strWhere, objValues);
            if (ds == null)
                return null;
			return DataTableToList(ds.Tables[0]);
		}
		/// <summary>
		/// 获得数据列表
		/// </summary>
		public List<EDRS.Model.YX_DZJZ_LSAJBD> DataTableToList(DataTable dt)
		{
			List<EDRS.Model.YX_DZJZ_LSAJBD> modelList = new List<EDRS.Model.YX_DZJZ_LSAJBD>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				EDRS.Model.YX_DZJZ_LSAJBD model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = dal.DataRowToModel(dt.Rows[n]);
					if (model != null)
					{
						modelList.Add(model);
					}
				}
			}
			return modelList;
		}

		/// <summary>
		/// 获得数据列表
		/// </summary>
		public DataSet GetAllList()
		{
			return GetList("");
		}

		/// <summary>
		/// 分页获取数据列表
		/// </summary>
        public int GetRecordCount(string strWhere, params object[] objValues)
		{
            return dal.GetRecordCount(strWhere, objValues);
		}
		/// <summary>
		/// 分页获取数据列表
		/// </summary>
        public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex, params object[] objValues)
		{
            return dal.GetListByPage(strWhere, orderby, startIndex, endIndex, objValues);
		}
        /// <summary>
        /// 律师阅卷登录
        /// </summary>
        public DataSet GetModelByZH(string yjzh, string yjmm)
        {
            return dal.GetModelByZH(yjzh, yjmm);

        }
        /// <summary>
        /// 律师阅卷登录
        /// </summary>
        public DataSet GetYZYJZH(string strWhere, params object[] objValues)
        {
            return dal.GetYZYJZH(strWhere,objValues);

        }
		/// <summary>
		/// 分页获取数据列表
		/// </summary>
		//public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		//{
			//return dal.GetList(PageSize,PageIndex,strWhere);
		//}
       
		#endregion  BasicMethod
		#region  ExtensionMethod

		#endregion  ExtensionMethod
	}
}

