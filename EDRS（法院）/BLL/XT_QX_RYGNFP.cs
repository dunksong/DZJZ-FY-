using System;
using System.Data;
using System.Collections.Generic;
using EDRS.Common;
using EDRS.Model;
using EDRS.DALFactory;
using EDRS.IDAL;
namespace EDRS.BLL
{
	/// <summary>
	/// XT_QX_RYGNFP
	/// </summary>
	public partial class XT_QX_RYGNFP
	{
		private readonly IXT_QX_RYGNFP dal=DataAccess.CreateXT_QX_RYGNFP();
        public XT_QX_RYGNFP(System.Web.HttpRequest _context)
        {
            dal.SetHttpContext(_context);
        }
		#region  BasicMethod
		/// <summary>
		/// 是否存在该记录
		/// </summary>
		public bool Exists(string DWBM,string GH,string GNBM)
		{
			return dal.Exists(DWBM,GH,GNBM);
		}

		/// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(EDRS.Model.XT_QX_RYGNFP model)
		{
			return dal.Add(model);
		}
        /// <summary>
		/// 增加多条数据
		/// </summary>
        public bool AddList(List<EDRS.Model.XT_QX_RYGNFP> modelList)
        {
            return dal.AddList(modelList);
        }
		/// <summary>
		/// 更新一条数据
		/// </summary>
		public bool Update(EDRS.Model.XT_QX_RYGNFP model)
		{
			return dal.Update(model);
		}

		/// <summary>
		/// 删除一条数据
		/// </summary>
		public bool Delete(string DWBM,string GH,string GNBM)
		{
			
			return dal.Delete(DWBM,GH,GNBM);
		}

        /// <summary>
        /// 删除一条数据
        /// </summary>
        public bool DeleteList(string listId)
        {
            return dal.DeleteList(listId);
        }

		/// <summary>
		/// 得到一个对象实体
		/// </summary>
		public EDRS.Model.XT_QX_RYGNFP GetModel(string DWBM,string GH,string GNBM)
		{
			
			return dal.GetModel(DWBM,GH,GNBM);
		}

		/// <summary>
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public EDRS.Model.XT_QX_RYGNFP GetModelByCache(string DWBM,string GH,string GNBM)
		{
			
			string CacheKey = "XT_QX_RYGNFPModel-" + DWBM+GH+GNBM;
			object objModel = EDRS.Common.DataCache.GetCache(CacheKey);
			if (objModel == null)
			{
				try
				{
					objModel = dal.GetModel(DWBM,GH,GNBM);
					if (objModel != null)
					{
						int ModelCache = EDRS.Common.ConfigHelper.GetConfigInt("ModelCache");
						EDRS.Common.DataCache.SetCache(CacheKey, objModel, DateTime.Now.AddMinutes(ModelCache), TimeSpan.Zero);
					}
				}
				catch{}
			}
			return (EDRS.Model.XT_QX_RYGNFP)objModel;
		}

		/// <summary>
		/// 获得数据列表
		/// </summary>
		public DataSet GetList(string strWhere)
		{
			return dal.GetList(strWhere);
		}
		/// <summary>
		/// 获得数据列表
		/// </summary>
        public List<EDRS.Model.XT_QX_RYGNFP> GetModelList(string strWhere, params object[] objValues)
		{
		    DataSet ds = dal.GetList(strWhere, objValues);
			return DataTableToList(ds.Tables[0]);
		}
		/// <summary>
		/// 获得数据列表
		/// </summary>
		public List<EDRS.Model.XT_QX_RYGNFP> DataTableToList(DataTable dt)
		{
			List<EDRS.Model.XT_QX_RYGNFP> modelList = new List<EDRS.Model.XT_QX_RYGNFP>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				EDRS.Model.XT_QX_RYGNFP model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = dal.DataRowToModel(dt.Rows[n]);
					if (model != null)
					{
						modelList.Add(model);
					}
				}
			}
			return modelList;
		}

		/// <summary>
		/// 获得数据列表
		/// </summary>
		public DataSet GetAllList()
		{
			return GetList("");
		}

		/// <summary>
		/// 分页获取数据列表
		/// </summary>
		public int GetRecordCount(string strWhere)
		{
			return dal.GetRecordCount(strWhere);
		}
		/// <summary>
		/// 分页获取数据列表
		/// </summary>
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			return dal.GetListByPage( strWhere,  orderby,  startIndex,  endIndex);
		}
		/// <summary>
		/// 分页获取数据列表
		/// </summary>
		//public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		//{
			//return dal.GetList(PageSize,PageIndex,strWhere);
		//}

		#endregion  BasicMethod
		#region  ExtensionMethod

		#endregion  ExtensionMethod
	}
}

